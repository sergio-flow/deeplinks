...
YUJ	America/Cambridge_Bay
YUK	Asia/Yerevan
YUL	America/Toronto
YUM	America/Phoenix
YUN	America/Yellowknife
YUQ	America/Cambridge_Bay
YUS	Asia/Shanghai
...
def parse_datetime_with_airport_timezone(date_str):
    iata_to_timezone = {}
    for line in iata.split("\n"):
        parts = line.strip().split('\t')
        if len(parts) == 2:
            iata_code, timezone_str = parts
            iata_to_timezone[iata_code] = timezone_str

    # Add IAH (Houston) if it's not in the file
    if 'IAH' not in iata_to_timezone:
        iata_to_timezone['IAH'] = 'America/Chicago'

    # Parse the input string
    pattern = r"(\w{3})\s+(\d{2})(\w{3})\s+(\d{1,2})(\d{2})([AP])"
    match = re.match(pattern, date_str)

    if not match:
        raise ValueError(f"Could not parse date string: {date_str}")

    airport_code, day, month_str, hour, minute, ampm = match.groups()

    # Get the timezone for this airport
    if airport_code not in iata_to_timezone:
        raise ValueError(f"Unknown airport code: {airport_code}")

    timezone_str = iata_to_timezone[airport_code]
    tz = pytz.timezone(timezone_str)

    # Convert to 24-hour format
    hour_24 = int(hour)
    if ampm.upper() == "P" and hour_24 < 12:
        hour_24 += 12
    elif ampm.upper() == "A" and hour_24 == 12:
        hour_24 = 0
    ...
    # Current year
    current_year = 2025  # Hardcoded to ensure consistency

    # Create the datetime in the local timezone first
    naive_dt = datetime(
        year=current_year,
    ...
    # Localize properly (accounting for DST)
...
# Solution for 3 parts or more.
def process_iten(iten_str, base_url):
    # Split by lines and process
    lines = iten_str.strip().split('\n')
    processed_lines = []
    ...
    for line in lines:
        ...
        if '(' in line and ')' in line:
            ...
            if left_paren < right_paren:
                cleaned_line = line[:left_paren] + line[right_paren+1:]

        # Trim whitespace
        cleaned_line = cleaned_line.strip()

        # Add to processed lines if there's content left
        if cleaned_line:
            processed_lines.append(cleaned_line)

    # Join all processed lines and remove any spaces
    final_iten = ''.join(processed_lines).replace(' ', '')

    # Create the final URL
    final_url = base_url + final_iten
...
def generate_iten(flight_details):
    ...
    for idx, part_key in enumerate(part_keys):
        ...
        if idx == 0:
            itin_summary.append(f"{origin} (itn[0].origin)")
